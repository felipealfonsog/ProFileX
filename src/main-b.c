...
static void activate(GtkApplication *app, gpointer user_data) {
    ...
    gtk_window_set_default_size(GTK_WINDOW(window), 600, 400); // Tamaño actualizado
    ...
    // Inicializar current_path si es NULL
...
static void update_file_manager_view(GtkWidget *window) {
    GtkWidget *scroll_window = gtk_bin_get_child(GTK_BIN(gtk_widget_get_parent(GTK_WIDGET(window))));
...
static void on_back_clicked(GtkButton *button, gpointer user_data) {
    ...
    if (previous_path) {
        ...
        previous_path = NULL; // Reset previous path
...
static void on_forward_clicked(GtkButton *button, gpointer user_data) {
    ...
    if (next_path) {
        ...
        g_free(next_path);
        next_path = NULL; // Reset next path
        update_file_manager_view(window);
    }
}
